...
XANVER=($($A12COINCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
...
for cmd in $A12COIND $A12COINCLI $A12COINTX $A12COINQT; do
  cmdname="${cmd##*/}"
  help2man -N --version-string=${XANVER[0]} --include=footer.h2m -o ${MANDIR}/${cmdname}.1 ${cmd}
  sed -i "s/\\\-${XANVER[1]}//g" ${MANDIR}/${cmdname}.1
done
