...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete A12coin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no A12coin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old A12coin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected A12coin 0.7 datadir."
    ;;
  3)
    echo "Detected A12coin pre-0.8 datadir."
    ;;
  4)
    echo "Detected A12coin 0.8 datadir."
    ;;
esac
