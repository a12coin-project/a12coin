...
*Setup instructions for a Gitian build of A12coin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the A12coin
...
- [Building A12coin Core](#building-a12coin-core)
...
Clone the git repositories for a12coin and Gitian.
...
in the a12coin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building A12coin Core
----------------

To build A12coin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the a12coin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/a12coin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/a12coin-project/a12coin
...
For example:
```bash
URL=https://github.com/thrasher-/a12coin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit a12coin=${COMMIT} --url a12coin=${URL} ../a12coin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit a12coin=${COMMIT} --url a12coin=${URL} ../a12coin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit a12coin=${COMMIT} --url a12coin=${URL} ../a12coin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the a12coin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../a12coin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/a12coin
SIGPATH=/some/root/path/a12coin-detached-sigs

./bin/gbuild --url a12coin=${BTCPATH},signature=${SIGPATH} ../a12coin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/a12coin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/a12coin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/a12coin-osx-build.assert
```
...
[a12coin-project/gitian.sigs](https://github.com/a12coin-project/gitian.sigs/) repository, or if that's not possible create a pull
