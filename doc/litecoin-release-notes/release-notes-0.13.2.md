A12coin Core version 0.13.2 is now available from:

  <https://download.a12coin.org/a12coin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/a12coin-project/a12coin/issues>
...
updates will be released anymore. Without security updates, using a a12coin
...
ECDSA signatures inside A12coin transactions now use validation using
...
Previous versions of A12coin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of A12coin Core was to raise the default minimum
relay fee.

A12coin Core 0.13.2 will have a strict maximum size on the mempool. The
...
A12coin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running a12coind without having to do any manual configuration.
...
A12coin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. A12coin Core will relay transactions with
...
entirely in A12coin Core 0.13, and it is at this time undecided whether the
...
A12coin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
A12coin Core automatically creates a hidden service to listen on, without
manual configuration. A12coin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if A12coin Core is listening, and
...
A12coind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals A12coin
Core to use floating fees. By default, A12coin Core will use floating
fees.
...
(default: `0.0002` XAN/kB).

At all times, A12coin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) XAN.
Furthermore, A12coin Core will never create transactions paying less than
...
`a12coin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 37332 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 37332 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /a12coinrpc>
        ...
        # optional bypass a12coind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< a12coinrpc:<password>
    </Location>
...
Restarting a12coind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in a12coin.conf
...
For example:
    ...
    $ src/a12coin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The A12coin Core code base has
...
- `a12coin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `a12coin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the A12coin scripting system that in combination with
...
A12coin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of A12coin Core.
...
The two phases are distinct in the A12coin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming A12coin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for A12coin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each A12coin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of a12coins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of a12coins being spent is exactly
...
- **Increased security for multisig:** A12coin addresses (both P2PKH addresses
  ...
  believed to be provided by A12coin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of A12coin's current ledger (called the UTXO set),
  ...
  note that A12coin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  A12coin users to individually opt-in to almost any change in the A12coin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and A12coin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the A12coin
client to A12coin Core 0.13.2 to signal for segwit support. When a
...
Since A12coin Core 0.10.0, nodes have defaulted to only relaying and
...
- `a12coin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `a12coin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
